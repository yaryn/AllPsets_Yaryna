...
def search():
    ...
    # TODO
    # retrieve q from HTML
    
    ## db.execute("SELECT * FROM places WHERE postal_code = :q", q=request.args.get("q")) ## ->
    ## -> but query requires that a user’s input be exactly equal to a postal code (per the =), 
    ## which isn’t all that compelling for autocomplete ( + is Python’s concatenation operator). So ->
    
    # -> appends % to the user’s input, which happens to be SQL’s "wildcard" character that means "match any number of characters
    q = request.args.get("q") + "%"
    place = db.execute("SELECT * FROM places WHERE postal_code LIKE :q OR place_name LIKE :q OR admin_name1 LIKE :q", q=q)
                                
    # Return about 10 places found as JSON object
    if len(place) > 10:
